				...
				<div class="box">
						...
						<div class="row" style="margin-top: 10px;">
							...
							<div class="col-sm-11">
								<select name="type" style="height: 25px;">
									...
									<option value="N" <c:out value="${pageMaker.cri.type eq 'N'? 'selected':'' }" />>주문자</option>
					...
					<div class="box-body">
						<div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
							...
							<div class="row">
								<div class="col-sm-12">
									<table id="orderlist" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
										<thead>
											<tr role="row">
											...
											<c:forEach items="${orderList }" var="orderVO" varStatus="status">
												<tr role="row" class="<c:if test="${status.count % 2 == 0 }">odd</c:if><c:if test="${status.count % 2 != 0 }">even</c:if>">
													...
													<td>
														...
														<input type="button" name="btnOrderDetail" value="상세보기" data-ord_code="${orderVO.ord_code }">
													</td>
												</tr>
											</c:forEach>
										</tbody>
							...
							<div class="row">
								<div class="col-sm-12">
									...
									<input type="button" id="btnCheck" value="엑셀다운">
								</div>
							</div>
						</div>
					</div>
...
<script>
	...
	$(document).ready(function(){
		...
		$(".paginate_button a").on("click",function(e) {
			...
			actionForm.append("<input type='hidden' name='ord_state' value='${ord_state}'>");	  // 주문상태정보 추가											
	...
	$("input[name='btnOrderDetail']").on("click",function() {
		...
		tr.next(".tr_detail").remove();                               //상품상세내용을 가지고 있는 추가된 tr태그를 화면에서 사라지게 함
		
		$("#orderlist tr.tr_detail").css("display", "none");
		
		tr.next(".tr_detail").css("dispaly", '');                     // 선택한 주문상세 tr에 화면에 나타나게 함.
